...
export default function UserDashboard() {
  ...
  const handleReorder = (order: any) => {
    ...
    order.items.forEach((item: any) => {
      addToCart({
        id: item.name, // using name as id for mock
        name: item.name,
        price: item.price,
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 w-full max-w-7xl mx-auto min-h-[70vh]">
      {/* Sidebar */}
      <div className="w-full md:w-64 bg-white rounded-3xl border border-slate-200 p-4 shrink-0 flex flex-col h-max">
      ...
      {/* Main Content */}
      <div className="flex-1 bg-white rounded-3xl border border-slate-200 p-6 sm:p-8">
        <AnimatePresence mode="wait">
            ...
            <motion.div key="orders" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              ...
              <div className="flex flex-col gap-4">
                {pastOrders.map(order => (
                  <div key={order.id} className="border border-slate-100 rounded-2xl p-5 hover:shadow-md transition-shadow">
                    <div className="flex flex-wrap justify-between items-center mb-4 gap-4">
                      ...
                      <div className="text-right">
                        <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full text-sm font-bold">{order.status}</span>
                    ...
                    <div className="border-t border-slate-100 pt-4 flex flex-wrap justify-between items-center gap-4">
                      <div className="text-sm text-slate-600">
                        {order.items.map(item => (
                          <div key={item.name}>{item.qty}x {item.name}</div>
                        ))}
                      </div>
                      <button 
                        onClick={() => handleReorder(order)}
                        className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-xl font-semibold hover:bg-teal-600 transition-colors"
                      >
            ...
            <motion.div key="wishlist" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {wishlistItems.map(item => (
                  <div key={item.id} className="border border-slate-100 rounded-2xl p-5 hover:shadow-md transition-shadow flex flex-col justify-between">
                    <div>
                      {item.priceDrop && (
                        <span className="flex items-center gap-1 text-xs font-bold text-emerald-600 bg-emerald-50 w-fit px-2 py-1 rounded-md mb-2">
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        <span className={`text-xs font-bold px-2 py-1 rounded ${item.stock === 'In Stock' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                          {item.stock}
                        </span>
                      </div>
                    </div>
                    <button 
                      ...
                      className="mt-4 w-full flex items-center justify-center gap-2 bg-slate-100 text-slate-700 px-4 py-2 rounded-xl font-semibold hover:bg-slate-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
          ...
          {activeTab === "notifications" && (
            <motion.div key="notifications" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              <h2 className="text-2xl font-bold text-slate-800 mb-6">Notification Center</h2>
              <div className="flex flex-col gap-3">
                {notifications.map(notif => (
                  <div key={notif.id} className={`p-4 rounded-2xl border ${notif.unread ? 'bg-teal-50/50 border-teal-100' : 'bg-white border-slate-100'} flex gap-4`}>
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${notif.unread ? 'bg-teal-100 text-teal-600' : 'bg-slate-100 text-slate-500'}`}>
                      <Bell className="w-5 h-5" />
                    </div>
                    <div>
                      <h3 className={`font-bold ${notif.unread ? 'text-teal-900' : 'text-slate-800'}`}>{notif.title}</h3>
                      <p className="text-sm text-slate-600 mt-0.5">{notif.message}</p>
                      <span className="text-xs text-slate-400 mt-2 block">{notif.date}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab === "loyalty" && (
            <motion.div key="loyalty" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col items-center py-8 text-center">
              <div className="w-24 h-24 bg-amber-50 rounded-full flex items-center justify-center mb-6 relative">
                <Award className="w-12 h-12 text-amber-500" />
                <span className="absolute -bottom-2 -right-2 bg-slate-900 text-white text-xs font-bold px-2 py-1 rounded-lg">GOLD</span>
              </div>
              <h2 className="text-3xl font-black text-slate-800 mb-2">1,250 Points</h2>
              <p className="text-slate-500 mb-8 max-w-md">You are a Gold member! You earn 2x points on every purchase and get early access to sales.</p>
              
              <div className="w-full max-w-md bg-slate-50 rounded-2xl p-6 border border-slate-100">
                <h3 className="font-bold text-slate-800 mb-4 text-left">Available Rewards</h3>
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-200">
                    ...
                    <button className="text-sm font-bold text-teal-600 bg-teal-50 px-3 py-1 rounded-lg hover:bg-teal-100">Redeem (500 pts)</button>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-200">
                    <div className="flex items-center gap-3">
                    ...
                    <button className="text-sm font-bold text-teal-600 bg-teal-50 px-3 py-1 rounded-lg hover:bg-teal-100">Redeem (800 pts)</button>
                  </div>
                </div>
              </div>
            </motion.div>
